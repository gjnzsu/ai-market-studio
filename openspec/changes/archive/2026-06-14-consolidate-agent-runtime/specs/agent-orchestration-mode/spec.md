...
**Reason**: Legacy orchestration is no longer a supported chat runtime. The workflow/playbook runtime is the single product direction for chat.
**Migration**: Existing clients should send `message` and optional `history` only. Simple data requests are handled by workflow tools such as `collect_market_context`.

### Requirement: Agent workflow mode is explicit opt-in
**Reason**: Chat no longer supports mode selection. Workflow behavior is always used.
**Migration**: Remove `agent_mode` from client requests and remove UI/API affordances for choosing legacy versus workflow mode.

### Requirement: Disabled workflow mode is predictable
**Reason**: Workflow mode is no longer feature-gated for chat, so disabled workflow behavior no longer exists.
**Migration**: Remove `ENABLE_AGENT_WORKFLOW_MODE` from environment configuration and documentation.

### Requirement: Tool exposure is mode-specific
**Reason**: There is only one chat runtime, so tool exposure is no longer selected by mode.
**Migration**: Assert the single model-facing tool set contains only approved intent-level workflow tools.

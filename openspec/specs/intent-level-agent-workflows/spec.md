...
The system SHALL provide a market briefing workflow that coordinates the required internal steps for a multi-source market explanation.
...
The system SHALL return structured workflow results that can be carried through the existing chat response contract.
